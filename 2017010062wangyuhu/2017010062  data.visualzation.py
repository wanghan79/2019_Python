...
class DataVisualization:
    ...
    @staticmethod
    def draw_plot(x, y0, y1, target):  # target为目标数据可为random_int或者random_float
    ...
    @staticmethod
    def draw_bar(x, y0, y1, target, style):  # target为目标数据，可为random_int或者random_float,style为样式，可为vertical或者horizontal
        fig, ax1 = plt.subplots()
        if target == 'random_int':
            if style == 'vertical':
    ...
    @staticmethod
    def draw_hist(y0, y1, target):  # target为目标数据可为random_int或者random_float
        num_bins = 50
        fig, ax1 = plt.subplots()
        if target == 'random_int':
    ...
    @staticmethod
    def draw_scatter(x, y0, y1, target):  # target为目标数据可为random_int或者random_float
        fig, ax1 = plt.subplots()
